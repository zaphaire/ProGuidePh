...
<div class="page-header">
    <div><h1>Edit Page: {{ $page->title }}</h1></div>
        ...
        <div>
            <div class="admin-card">
                ...
                <div class="form-group">
                    ...
                    <textarea name="body" id="page-editor" class="form-control" rows="15">{{ old('body', $page->body) }}</textarea>
                </div>
            </div>
        </div>
        <div>
            <div class="admin-card">
                <div class="form-group">
                    ...
                    <input type="number" name="order" value="{{ old('order', $page->order) }}" class="form-control" min="0">
                </div>
                <div class="form-group">
                ...
                <div style="display:flex;align-items:center;gap:.75rem;padding:.75rem;background:rgba(16,185,129,.05);border-radius:8px;border:1px solid rgba(16,185,129,.15);margin-bottom:1.25rem">
                    <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $page->is_published) ? 'checked' : '' }} style="width:18px;height:18px;accent-color:#10b981">
                    <label for="is_published" style="color:#10b981;font-weight:600;cursor:pointer">Published</label>
                </div>
                <button type="submit" class="btn btn-primary-admin" style="width:100%">Save Changes</button>
            </div>
        </div>
    </div>
</form>
